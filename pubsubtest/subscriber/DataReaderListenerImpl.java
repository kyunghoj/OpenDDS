...
public class DataReaderListenerImpl extends DDS._DataReaderListenerLocalBase {
    ...
    public synchronized void on_data_available(DDS.DataReader reader) {
        ...
        MsgDataReader mdr = MsgDataReaderHelper.narrow(reader);
        if (mdr == null) {
            System.err.println("ERROR: read: narrow failed.");
            return;
        }

        MsgHolder mh = new MsgHolder(new Msg());
        ...
        if (status == RETCODE_OK.value) {
            ...
            if (sih.value.valid_data) {
                ...
                if (mh.value.from.equals("FruitPublisher")) {
                    if (mh.value.text.equals("Pick Veges")) {
                        System.out.println("COORDINATION: Time to subscribe VegePublisher.");
                    }
                }
